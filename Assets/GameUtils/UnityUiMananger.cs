...
namespace Assets.GameUtils
{
    public class UnityUiMananger : IUiManager
    {
		...
		public void deletePion(int x, int y)
		{
			...
			if(GameObject.Find ("inter_" + x + "_" + y).transform.FindChild ("pion").renderer.material.color == Color.white)
			{
				GameObject.Find ("inter_" + x + "_" + y).transform.FindChild ("pion").renderer.material.color = Color.blue;
			}else
			{
		...
		public void DisplayToolTip(string s)
		{
				...
				if(leGroupe != null)
				{
					...
					if(GameObject.Find(String.Format("inter_{0}_{1}",inter.Coord.X,inter.Coord.Y)).transform.childCount > 0 && GameObject.Find (String.Format ("inter_{0}_{1}", inter.Coord.X, inter.Coord.Y)).transform.GetChild (0).renderer.material.color != Color.red && GameObject.Find (String.Format ("inter_{0}_{1}", inter.Coord.X, inter.Coord.Y)).transform.GetChild (0).renderer.material.color != Color.blue)
						{
